...
class SECSParser:
    """SEMI E5 SECS-II Data Item Parser with Hex Wrap & Cascade List View"""
    FORMAT_CODES = {
    ...
    @classmethod
    def parse_cascade(cls, data, indent=0):
        """계층 구조(Cascade) 리스트 형태로 파싱"""
        ...
        try:
            ...
            if fmt_name == "L":
                ...
                for _ in range(length):
                    txt, consumed = cls.parse_cascade(sub_data[offset:], indent + 1)
                    out += txt
                    offset += consumed
                out += f"{spacing}>\n"
...
class HSMSInstance(QObject):
    ...
    def handle_secs_message(self, header, payload):
        ...
        # 2. 캐스케이드 리스트 출력
        cascade_view, _ = SECSParser.parse_cascade(payload)
    ...
    async def send_data_message(self, s, f, payload_text=""):
        ...
        # 전송 시 주석(*) 처리
        clean_lines = [line.split('*')[0].strip() for line in payload_text.splitlines() if line.split('*')[0].strip()]
        final_payload = "\n".join(clean_lines).encode('utf-8')

        if self.transport:
            full = header.pack() + final_payload
            self.transport.write(struct.pack(">I", len(full)) + full)
            self.log_signal.emit(f"□ SEND [S{s}F{f}] Payload: {len(final_payload)} bytes")
        else:
...
class HSMSMonitorApp(QMainWindow):
    ...
    def init_ui(self):
        self.setWindowTitle("HSMS Master v2.5.5 - Cascade View")
        ...
        self.in_payload = QTextEdit(); self.in_payload.setFixedHeight(100)
        btn_send = QPushButton("SEND DATA MESSAGE"); btn_send.setStyleSheet("background:#27ae60; color:white; font-weight:bold;")
        ...
        self.log_view.setLineWrapMode(QTextEdit.LineWrapMode.NoWrap) # 가로 구조 유지
    ...
    def on_node_selected(self, idx):
        if idx <= 0: return
        name = self.combo_nodes.currentText(); self.current_node_name = name
